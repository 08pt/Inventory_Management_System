...
router.post("/register", async (req, res) => {
    try {
        ...
        UserModel.findOne({ email: userModel.email }, function (_err, user) {
            if (user) {
...
router.post('/verifyemail', async function (req, res) {
    try {
        let userModel = new UserModel(req.body)
        await UserModel.findOne({ email: userModel.email }, function (_err, user) {
            if (user) {
...
router.post('/verifyToken', async function (req, res) {
    try {
        await jwt.verify(req.body.token, config.secret, (err, _decoded) =>{
            if (err)
